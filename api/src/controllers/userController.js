...
module.exports = class userController {
  static async createUser(req, res) {
    ...
    const nascimentos = new Date (data_nascimento);
    ...
    else if (nascimentos >= new Date){
      return res.status(400).json({ error: "Data de nascimento ivalida" });
    }
    else{
...
//UPDATE
  ...
  static async loginUser(req, res) {
    ...
    const querry = 'SELECT * FROM usuario where email = ?'

    try {
      connect.query(querry,[email],(err,results)=>{
        if(err){
          console.log(err)
          return res.status(500).json ({error: "Erro inerno do servidor"})
        }
        if(results.length === 0){
